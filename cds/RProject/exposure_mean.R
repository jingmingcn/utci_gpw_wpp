...
# requried: gpw file path ???
gpw_file_path <- ""
gpw <- brick(gpw_file_path)

# gpw must be cropped 
gpw <- crop(gpw,extent(-180,180,-60,90))

# required: countries file path ???
countries_file_path <- ""
countries <- read.xlsx(countries_file_path,sheetIndex = 1)

# required: wpp file path
wpp_file_path <- "" ???
wpp <- read_xlsx(wpp_file_path,skip=16,na="...")

#  required: the folder holds the utci group data generated by the process of the step 2 ???
